...
select dvdk.ma_dich_vu_di_kem as ID, dvdk.ten_dich_vu_di_kem as Name
...
select hd.ma_hop_dong, nv.ho_ten AS nhan_vien, kh.ho_ten AS khach_hang, kh.so_dien_thoai, dvdk.ten_dich_vu_di_kem, hd.tien_dat_coc
from hop_dong hd
...
where hd.ngay_lam_hop_dong between '2020-10-01' and '2020-12-31'
  and not exists (
      ...
      join hop_dong_chi_tiet hdct2 
        on hd2.ma_hop_dong = hdct2.ma_hop_dong
      where hd2.ngay_lam_hop_dong between '2021-01-01' and '2021-06-30'
        and hdct2.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
  )
...
select dvdk.ma_dich_vu_di_kem, dvdk.ten_dich_vu_di_kem, COUNT(*) AS so_lan_su_dung
from hop_dong_chi_tiet hdct
join dich_vu_di_kem dvdk on hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
group by dvdk.ma_dich_vu_di_kem, dvdk.ten_dich_vu_di_kem
having count(*) = (select max(so_lan)
    from (
...
# 14. Hiển thị thông tin tất cả các Dịch vụ đi kèm chỉ mới được sử dụng một lần duy nhất. Thông tin hiển thị bao gồm ma_hop_dong, ten_loai_dich_vu, ten_dich_vu_di_kem, so_lan_su_dung (được tính dựa trên việc count các ma_dich_vu_di_kem).alter
